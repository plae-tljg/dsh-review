...
export function ReviewPanelBody({ scope, tab, face, store, t, openFile }) {
  ...
  const select = useMemo(() => (id, path, held, context) => { bridge.select(id, path, held, context, controller.signal) }, [bridge, controller])
  const open = useMemo(() => (id, path, context) => { bridge.open(id, path, context, controller.signal) }, [bridge, controller])
  ...
  const openCommitDiff = useMemo(() => (id, oid, path, context) => { bridge.openCommitDiff(id, oid, path, context, controller.signal) }, [bridge, controller])
